...
index = 0;                                  // определяем индекс первого элемента массива, выводимый на печать
Console.WriteLine();
Console.WriteLine("Массив из 20 чисел от -25 до +25 включительно:");
while (index < 20)                          // задаем условие для цикла выведения элементов массива на печать: пока индекс меньше 20
{
    int val = A[index];                     // определяем текущий элемент для вывод на печать
    Console.Write(val + " ");               // выводим на печать элемент массива
    index++;                                // определяем индекс следующего элемента
}
Console.WriteLine();
...
int current = A[0];                         // определяем первый элемент списка (первый текущий элемент)
Console.WriteLine();                        
Console.WriteLine("Элементы, не нарушающие порядок возрастания:");
Console.Write(current + " ");               // выводим первый элемент списка на печать
index = 1;                                  // определяем первый проверяемый элемент массива (по индексу)
while (index <= 19)                         // определяем цикл: для каждого следующего элемента массива
{
...
// для этого сравниваем каждый элемент с числом 8, в список включаем те, которые больше

Console.WriteLine();
Console.WriteLine("Элементы массива, которые больше 8:");
for (index = 0; index < 20; index++)        // определяем цикл: для каждого следующего элемента массива
...
for (index = 0; index < 20; index++)        // определяем цикл: для каждого следующего элемента массива
                                            // тело цикла:
    if (cur < 0 && A[index] > 0)            // задаем первое условие: текущий элемент должен быть меньше 0, а следущий элемент - больше 0
    {
        ...
        {
            if (cur > 0 && A[index] < 0)    // задаем второе условие: текущий элемент должен быть больше 0, а следующий элемент - меньше 0
            {
